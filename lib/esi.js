...
function ESI(config) {
    ...
    function processHtmlText(html, options, state) {
        ...
        let i = 0;
        const includeTags = findESIIncludeTags(html);
        const varsTags = findESIVarsTags(html);
        if (!includeTags.length && !varsTags.length) {
        ...
        if(varsTags.length) {
            varsTags.forEach(tag => {
                const placeholder = '<!-- esi-placeholder-' + i + ' -->';
                if(maxDepthReached) {
                    html = html.replace(tag, '');
                } else if(tag.includes('<esi:vars')) {
                    html = html.replace(tag, placeholder);
                    const varName = getBoundedString('name="', '"')(tag);
                    const varValue = options.vars && options.vars[varName];
                    if (varValue) {
                        html = html.replace(placeholder, () => varValue);
                    } else {
    ...
    function hasESITag(html) {
        return html.match(/<esi:include.*?(?:\/\s*>|<\/esi:include>)/gms) || html.match(/<esi:vars.*?(?:\/\s*>|<\/esi:vars>)/gms)
    }
    ...
    function findESIVarsTags(html) {
        const re = /<esi:vars.*?(?:\/\s*>|<\/esi:vars>)/gms;
        const tags = [];
        let match;
        while ((match = re.exec(html)) !== null) {
            tags.push(match[0]);
    ...
    function getVarsContent(tag, options) {
        const name = getBoundedString('name="', '"')(tag);
        return options.vars[name];
    }
